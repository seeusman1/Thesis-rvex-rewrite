...
\field{11..10}{M}
%-------------------------------------------------------------------------------
This register selects whether address translation by means of the MMU is enabled 
...
enable command to the M flag in \creg{SCCR} and subsequently executing 
\insn{RFI}. Neither the core nor the  debug bus can write to this field 
directly. This is because the core needs to be idle for address translation to 
be enabled or disabled.

When address translation is enabled, the register reads as \code{01}. When it is
disabled, it reads as \code{10}. If the processor is not design-time configured
to contain an MMU, this field and its counterpart in \creg{SCCR} still behave as
...
_r = _v;
\end{lstlisting}
\connect{cxreg2mem_mmuEnable}{_r}
...
\creg{SCCR} and subsequently executing \insn{RFI}. Neither the core nor the 
debug bus can write to this field directly.

In kernel mode, the register reads as \code{01}. In user mode, it reads as 
\code{10}.
...
_r = _v;
\end{lstlisting}
\connect{cxreg2mem_kernelMode}{_r}
...
translation was enabled or disabled. When \insn{RFI} is executed, the state is 
set to this value.

Unlike the M field in \creg{CCR}, this field can be written. Writing \code{00} 
has no effect, writing \code{01} enables address translation, writing \code{10} 
disables it and writing \code{11} toggles the state. This prevents the need for 
...
processor was in kernel mode or user mode. When \insn{RFI} is executed, the 
state is set to this value.

Unlike the kernel mode field in \creg{CCR}, this field can be written. Writing 
