...
# Parse command line.
if len(argv) != 3:
    print('Usage: python registers-latex.py <infile.tex> <outfile.tex>')
    exit(2)
infile = argv[1]
outfile = argv[2]

# Parse the input file.
regmap, regdoc = registers.parse(infile)

# Define LaTeX table stuff.
def print_reg_head(f, wrap=False):
    f.write('\\vskip -10 pt\\relax\\noindent\\footnotesize\n')
    f.write('\\begin{' + ('longtable' if wrap else 'tabular') + '}{@{}p{18mm}@{}*{32}{p{3.4mm}@{}}p{18mm}@{}}\n')
    f.write((' &' * 33) + ' \\\\\n')
    s = '\\multicolumn{1}{@{}l@{}|}{\\scriptsize Offset \\hfill \\hfill Bit \\ } & '
...
def print_empty_row(f, offset):
    ...
    s += '\\multicolumn{32}{@{}c@{}|}{\\footnotesize\\textit{Unused}} & \\\\\n'
    s += '\\cline{2-33}\n'
    f.write(s)

def print_reg_detail(f, rowname, values):
    s = '\\multicolumn{1}{@{}l@{}}{\\scriptsize \\hfill \\hfill ' + rowname + ' \\ } & '
