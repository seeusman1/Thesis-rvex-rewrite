...
/*
	serial_init();
	serial_setbrg();
	serial_puts("starting ray tracer\n");
*/
...
//	#if 0
	...
	for(y=start_height;y<end_height;y++){
		for(x=0;x<WIDTH;x++){
			...
			if (x < WIDTH-BARWIDTH)
				img[(x + y*WIDTH)] = (((char)min(red*255.0f, 255.0f) & 0xFF) << 16) | (((char)min(green*255.0f, 255.0f) & 0xFF) << 8) | ((char)min(blue*255.0f, 255.0f) & 0xFF);
			else
				img[(x + y*WIDTH)] = 0xff<<(get_core_ID()*8); //assuming the compiler recognizes the *8, otherwise change it to <<3
		}
	}
//	#endif
...
/*
	for (i = 0; i < 640*160; i++)
	{
		img[i] = 0x00FF0000 - ((((i%640)/3)&0xff)<<16) ;
	}
	
	for (; i < 640*320; i++)
	{
		img[i] = 0x0000FF00 - ((((i%640)/3)&0xff)<<8);
	}
	
	for (; i < 640*480; i++)
	{
		img[i] = 0x000000FF - (((i%640)/3)&0xff);
	}
*/
