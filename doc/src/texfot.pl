...
# Updated by Jeroen van Straten to output \subimport filenames as well.
...
sub process_output {
  ...
  LINE: while (<$fh>) {
    ...
    next if /^(
     ...
     |This\ is.*(epsf\.tex|\.sty|Version) # so what
    ...
    if (/^(
     ...
     |.*Token\ not\ allowed\ in\ a\ PDF\ string\ \(PDFDocEncoding\)
      ...
      if ($current_file_printed == 0) {
        print "\n\033[1mIn file ";
        print $current_file;
        print ":\033[0m\n";
        $current_file_printed = 1;
      }
      print $prefix;
      print " - ";
      print $line;
    ...
    if (/^(
     ...
     #|Output\ written
     |No\ pages\ of\ output
     #|Underfull          # I don't care about underfull stuff, what does that even mean
     |Overfull
